...
- 2026-07-24 short-fiction routing trace fix: aligned the strict persisted message route-result schema with the versioned trace already written by routing-summary transcript events. Added a transcript-to-session regression for traced results. Full pnpm 9.15.9 frozen install plus direct package build/typecheck/test/publish-manifest validation passed (Core 200 files / 1940 tests, Studio 64 files / 587 tests, CLI 41 files / 229 tests). Restarted Studio against `D:\inkos-data\default`; the entry and current assets returned HTTP 200, and both existing traced short-fiction sessions loaded with HTTP 200.
